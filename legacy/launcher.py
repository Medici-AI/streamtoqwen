...
def print_modes():
    ...
    print("  5. multi-agent - Multi-agent analysis (5 specialized agents)")
    print("  6. test       - Test connections only")
    print("  7. simulate   - Run with simulated data (no microphone)")
...
Examples:
  ...
  python launcher.py test
    ...
    parser.add_argument(
        "mode",
        choices=["console", "fast", "standard", "detailed", "multi-agent", "test", "simulate"],
        help="Launch mode"
    )
    ...
    modes = {
        ...
        "multi-agent": {
            "cmd": ["python", "main_elevenlabs_microphone_multi_agent.py",
                   "--agent-id", args.agent_id,
                   "--window-size", "10",
                   "--debug-style", args.debug_style,
                   "--chunk-type", "word"],
            "desc": "Starting Multi-Agent Mode (5 specialized agents, 10s windows)"
        },
        "test": {
            "cmd": ["python", "-c", """
import asyncio
...
asyncio.run(test())
    ...
    if args.window_size and args.mode in ["fast", "standard", "detailed", "multi-agent"]:
